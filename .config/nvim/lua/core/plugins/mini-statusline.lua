...
local function lua_line()
    ...
    if diagnostics_table[vim.diagnostic.severity.ERROR] ~= nil then
        table.insert(diagnostic_strings, color_text(error_count_status_line, "DiagnosticVirtualTextHint"))
    end
    if diagnostics_table[vim.diagnostic.severity.WARN] ~= nil then
        table.insert(diagnostic_strings, color_text(warning_count_status_line, "DiagnosticVirtualTextWarning"))
    end
    if diagnostics_table[vim.diagnostic.severity.INFO] ~= nil then
        table.insert(diagnostic_strings, color_text(info_count_status_line, "DiagnosticVirtualTextWarning"))
    end
    if diagnostics_table[vim.diagnostic.severity.HINT] ~= nil then
        table.insert(diagnostic_strings, color_text(hint_count_status_line, "DiagnosticVirtualTextInfo"))
    end
    ...
    return MiniStatusline.combine_groups({
        ...
        "%=", -- End left alignment
        { hl = "MiniStatuslineFilename", strings = { diagnostic_values } },
